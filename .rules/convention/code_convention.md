...
| 파일, 폴더 이름 | snake_case       | `user_card.ts`, `api_client/`, `use_auth.ts` |
...
- **컴포넌트 파일**: 컴포넌트 이름은 PascalCase이지만, 파일 이름은 snake_case로 작성한다.
  - ex) `UserCard` 컴포넌트 → `user_card.tsx`

## 훅 (Hooks)

- 파일 이름: snake_case → `use_auth.ts`
