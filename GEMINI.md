...
- **주요 책임**:
    ...
    - **최종 품질 보증**: Codex가 수행한 빌드 결과를 확인하고, 전체 기획 요구사항과의 정합성 및 에이전트 간 협업 결과물을 최종 승인.
    - **데이터 및 보안 관리**: DB 스키마 설계 및 RLS 정책 등 보안 아키텍처 수립.
    - **이슈 및 칸반 보드 관리**: 모든 작업 시작 전 이슈 정의, 적절한 **Labels(유형, 담당 에이전트, 상태)** 부여 및 **GitHub Projects(칸반 보드)**를 통한 진행 상황 시각화.
